import { createClickhouseClient, insert } from '@org/clickhouse_client';
import { createConsumerClient, type KafkaEachBatchPayload } from '@org/kafka_client';
...
async function main() {
	...
	const flush = async (commitOffsetsIfNecessary: () => Promise<void>) => {
		...
		await insert(clickhouse, config.clickhouseTable, rowsToWrite);
	...
	console.log(`analytics-consumer starting: topic=${config.kafkaTopic}, table=${config.clickhouseTable}`);
	...
	try {
		await consumer.run({
			...
			eachBatch: async ({ batch, resolveOffset, commitOffsetsIfNecessary, heartbeat, isRunning, isStale }: KafkaEachBatchPayload) => {
				for (const message of batch.messages) {
					...
					if (parsed === null) {
						console.warn(`Skipping invalid analytics message at ${batch.topic}[${batch.partition}] offset ${message.offset}`);
