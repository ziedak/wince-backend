...
// ─── QuickRecoveryDrawer ──────────────────────────────────────────────────────
//
// Drop-in component for LiveOps.jsx. Replaces the "Visitor Lookup" slot in the
// Actions Strip with a full-transparency manual intervention drawer.
//
// What this does differently from other panels:
//   • Pending Approvals  — AI proposes, merchant approves, per-session batch
//   • Visitor Drawer     — merchant acts, single session, immediate
//   • QuickRecovery      — merchant proposes, segment scope, immediate
//
// The session list is derived from ACTIVE_SESSIONS already in the file.
// Filtering is live: changing the target segment refreshes which sessions appear.
// Estimated recovery scales proportionally by selected cart value so partial
// selections show honest numbers, not inflated totals.
//
// ─── INTEGRATION CHECKLIST ────────────────────────────────────────────────────
//
// 1. Paste this component into LiveOps.jsx (before the App function).
//
// 2. Add state to App:
//      const [showQuickRecovery, setShowQuickRecovery] = useState(false);
//
// 3. Add to the App return, alongside other drawers:
//      {showQuickRecovery && (
//        <QuickRecoveryDrawer
//          onClose={() => setShowQuickRecovery(false)}
//          budgetLeft={budget.total - budget.used}
//        />
//      )}
//
// 4. Add to the ESC keydown handler:
//      setShowQuickRecovery(false);
//
// 5. In the Actions Strip, replace the Visitor Lookup div with:
//      <div className="self-stretch w-px bg-white/6" />
//      <div>
//        <div className="mb-2 text-xs font-semibold tracking-widest uppercase text-white/30"
//             style={{ ...mono, fontSize: "0.58rem" }}>Quick Recovery</div>
//        <button
//          onClick={() => setShowQuickRecovery(true)}
//          className="flex items-center gap-2 px-3 py-2 text-xs font-medium transition-all border rounded-xl border-amber-500/25 // bg-amber-500/6 text-amber-400/80 hover:border-amber-500/40 hover:text-amber-400"
//        >
...
function QuickRecoveryDrawer({
  ...
  const TARGETS = [
    {
      ...
      filter: (s: ActiveSession) =>
        s.stage === 'Checkout' && s.device === 'mobile',
    },
    {
      id: 'critical',
      label: 'Critical ≥80%',
      sub: 'highest exit risk right now',
      filter: (s: ActiveSession) => s.risk >= 80,
    },
    {
      id: 'cart-all',
      label: 'All Cart sessions',
      sub: 'price hesitation signals',
      filter: (s: ActiveSession) => s.stage === 'Cart',
    },
  ] as const
  ...
  type InterventionId = (typeof INTERVENTIONS)[number]['id']

  const [targetId, setTargetId] = useState<TargetId>('checkout-mobile')
  const [interventionId, setInterventionId] =
    useState<InterventionId>('urgency')
  ...
  const currentIntervention = INTERVENTIONS.find(
    (a) => a.id === interventionId
  )!
  const targetSessions = ACTIVE_SESSIONS.filter(currentTarget.filter)

  // When the target changes, select all sessions by default.
  // The merchant can then deselect exceptions they don't want to touch.
  useEffect(() => {
    setSelected(
      new Set(ACTIVE_SESSIONS.filter(currentTarget.filter).map((s) => s.id))
    )
  }, [targetId])

  const selectedSessions = targetSessions.filter((s) => selected.has(s.id))
  const allSelected =
    selectedSessions.length === targetSessions.length &&
    targetSessions.length > 0
  const someSelected = selectedSessions.length > 0 && !allSelected

  const toggleAll = () =>
    setSelected(
      allSelected ? new Set() : new Set(targetSessions.map((s) => s.id))
    )
  ...
  // Scale estimated recovery by proportion of selected cart value.
  // This avoids showing $1,200 recovery when only $300 of cart is selected.
  const totalCartAll = targetSessions.reduce((a, s) => a + s.cart, 0)
  const totalCartSelected = selectedSessions.reduce((a, s) => a + s.cart, 0)
  const estRecovery =
    totalCartAll > 0
      ? Math.round(
          currentIntervention.baseEst *
            targetSessions.length *
            (totalCartSelected / totalCartAll)
        )
      : 0

  const totalCost = currentIntervention.unitCost * selectedSessions.length
  const overBudget = totalCost > budgetLeft

  const canDeploy =
    phase === 'idle' && selectedSessions.length > 0 && !overBudget
  ...
  const deploy = () => {
    ...
    setTimeout(() => {
      setPhase('done')
      toast.success(
        `${currentIntervention.label} deployed to ${selectedSessions.length} session${selectedSessions.length !== 1 ? 's' : ''}`,
        { description: `Est. recovery +$${estRecovery}` }
      )
      setTimeout(() => {
        setPhase('idle')
        onClose()
  ...
  const deployBtnStyle =
    phase === 'done'
      ...
      : phase === 'deploying'
        ? 'bg-white/5 text-white/25 border border-white/8 cursor-wait'
        : !canDeploy
  ...
  const deployBtnText =
    phase === 'done'
      ? `✓ Deployed to ${selectedSessions.length} session${selectedSessions.length !== 1 ? 's' : ''}`
      : phase === 'deploying'
        ? 'Deploying…'
        : selectedSessions.length === 0
          ? 'Select sessions to deploy'
          : overBudget
            ? 'Budget exceeded — choose a free intervention'
            : `Deploy ${currentIntervention.label} to ${selectedSessions.length} session${selectedSessions.length !== 1 ? 's' : ''}`
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ width: 420, background: '#13161d' }}
      >
        {/* ── Header ── */}
        <div className='flex items-start justify-between px-5 py-4 border-b shrink-0 border-white/6'>
          <div>
            <div
              className='mb-1 font-semibold tracking-widest uppercase text-white/30'
              style={{ ...mono, fontSize: '0.58rem' }}
            >
              Manual Intervention
            </div>
            <div className='text-sm font-semibold text-white'>
              Choose sessions &amp; deploy
            </div>
            <div
              className='mt-0.5 text-white/30'
              style={{ fontSize: '0.65rem' }}
            >
              Bypasses the AI approval queue for immediate deployment
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* ── Target + Intervention pickers ── */}
        <div className='px-5 py-3 border-b shrink-0 border-white/6'>
          {/* Target segment */}
          <div className='mb-1.5 text-white/25' style={{ fontSize: '0.62rem' }}>
            Target segment
          </div>
          <div className='mb-3 flex gap-1.5'>
            {TARGETS.map((t) => {
              const count = ACTIVE_SESSIONS.filter(t.filter).length
              const isSel = t.id === targetId
              return (
                <button
                  key={t.id}
                  onClick={() => setTargetId(t.id)}
                  className={`flex-1 rounded-lg border px-2 py-1.5 text-center font-medium transition-colors ${
                    isSel
                      ? 'border-blue-500/30 bg-blue-500/10 text-blue-300'
                      : 'border-white/8 text-white/30 hover:border-white/12 hover:text-white/50'
                  }`}
                  style={{ fontSize: '0.62rem' }}
                >
                  {t.label}
                  <div
                    className={`mt-0.5 font-bold ${isSel ? 'text-blue-400/65' : 'text-white/18'}`}
                    style={{ ...mono, fontSize: '0.58rem' }}
                  >
          ...
          <div className='flex gap-1.5'>
            {INTERVENTIONS.map((a) => {
              const isSel = a.id === interventionId
              return (
                <button
                  key={a.id}
                  onClick={() => setInterventionId(a.id)}
                  className={`flex-1 rounded-lg border py-1.5 text-center font-medium transition-colors ${
                    isSel
                      ? 'border-amber-500/28 bg-amber-500/8 text-amber-300'
                      : 'border-white/8 text-white/30 hover:border-white/12 hover:text-white/50'
                  }`}
                  style={{ fontSize: '0.62rem' }}
                >
                  {a.label}
                  <div
                    className={`mt-0.5 ${isSel ? 'text-amber-400/50' : 'text-white/18'}`}
                    style={{ ...mono, fontSize: '0.56rem' }}
                  >
                    {a.sub}
                  </div>
                </button>
        ...
        <div className='flex-1 min-h-0 overflow-y-auto scrollbar-hide'>
          {/* Select-all row — sticky so it stays visible while scrolling */}
          <div
            className='sticky top-0 z-10 flex items-center gap-3 border-b border-white/5 px-5 py-2.5'
          ...
          >
            ...
            >
              {allSelected
                ? `All ${targetSessions.length} sessions selected`
                : someSelected
                  ? `${selectedSessions.length} of ${targetSessions.length} selected`
                  : `${targetSessions.length} session${targetSessions.length !== 1 ? 's' : ''}`}
            </span>
            <span
              className='ml-auto text-white/18'
              style={{ fontSize: '0.6rem' }}
            >
              Cart · In stage · AI status
            </span>
          </div>
          ...
          {targetSessions.length === 0 ? (
            <div
              className='px-5 py-10 italic text-center text-white/25'
              style={{ fontSize: '0.72rem' }}
            >
              No sessions match this segment right now
            </div>
          ) : (
            targetSessions.map((s, idx) => {
              const isSel = selected.has(s.id)
              const isLast = idx === targetSessions.length - 1
              const rc = riskColor(s.risk)
              const statusMeta = AI_STATUS_META[s.aiStatus]
              const isKnown = !s.visitor.startsWith('Guest')

              return (
                <div
                  key={s.id}
                  onClick={() => toggleOne(s.id)}
                  className={`flex cursor-pointer transition-colors ${!isLast ? 'border-b border-white/[0.04]' : ''} ${
                    isSel ? 'bg-blue-500/[0.05]' : 'hover:bg-white/[0.02]'
                  }`}
                >
                  {/* Left risk bar — coloured when selected, invisible when not */}
                  <div
                    className='w-0.5 shrink-0 self-stretch transition-all duration-150'
                    style={{ background: isSel ? rc + '65' : 'transparent' }}
                  />

                  <div className='flex items-center flex-1 min-w-0 gap-3 px-4 py-3'>
                    {/* Checkbox */}
                    ...
                    {/* Risk % — fixed column for scannability */}
                    <div className='w-8 text-right shrink-0'>
                      <span
                        className='font-bold'
                        style={{ ...mono, color: rc, fontSize: '0.72rem' }}
                      >
                        {s.risk}%
                      </span>
                    </div>

                    {/* Visitor identity + behavioural signal */}
                    <div className='flex-1 min-w-0'>
                      <div className='mb-0.5 flex items-center gap-1.5 overflow-hidden'>
                        <span
                          className={`truncate font-semibold ${isKnown ? 'text-white/85' : 'text-white/42'}`}
                          style={{ fontSize: '0.72rem' }}
                        >
                          {s.visitor}
                        </span>
                      ...
                      <div
                        className='italic truncate text-white/28'
                        style={{ fontSize: '0.62rem' }}
                      >
                        {s.signal}
                        {s.pendingAction && (
                          <span className='not-italic text-white/18'>
                            {' '}
                            · {s.pendingAction}
                          </span>
                        )}
                    ...
                    {/* Cart value + time in stage + AI status */}
        ...
        {/* ── Footer ── */}
        <div
          className='flex shrink-0 flex-col gap-2.5 border-t border-white/6 px-5 py-4'
          style={{ background: '#0f1117' }}
        >
          {/* Live impact summary — updates as sessions are checked/unchecked */}
          <div
            className='flex flex-wrap items-center gap-2'
            style={{ fontSize: '0.68rem', minHeight: 18 }}
          >
            {selectedSessions.length === 0 ? (
              <span className='text-white/20'>
                Select sessions to see impact estimate
              </span>
            ) : (
              <>
                <span className='text-white/30'>Deploying to</span>
                <span className='font-semibold text-white/65' style={mono}>
                ...
                {totalCost > 0 && (
                  <>
                    ...
                    <span
                      ...
                      style={{
                        ...mono,
                        color: overBudget ? '#f87171' : '#f59e0b',
                      }}
                    >
                      −${totalCost}
                    </span>
          ...
          {/* Budget guard — visible and specific so the merchant knows exactly why they're blocked */}
          {overBudget && (
            <div className='flex items-center gap-1.5 rounded-xl border border-red-500/20 bg-red-500/6 px-3 py-2'>
              <AlertTriangle size={10} className='text-red-400 shrink-0' />
              <span className='text-red-400/80' style={{ fontSize: '0.62rem' }}>
                Cost exceeds remaining budget (${budgetLeft} left). Deselect
                sessions or switch to a free intervention.
              </span>
            </div>
          )}
          ...
          <div className='flex gap-2'>
            <button
              onClick={deploy}
              disabled={phase !== 'idle' || !canDeploy}
              className={`flex-1 rounded-xl py-2.5 text-xs font-semibold transition-all ${deployBtnStyle}`}
            >
              {deployBtnText}
            </button>
            <button
