...
export class BudgetService {
  ...
  async checkAndReserve(
    ...
    try {
      ...
      if (!approved) {
        this.logger.info(
          { storeId, amount, maxAmount },
          'BudgetService: daily budget exhausted'
        )
      }
      return approved
    } catch (err) {
      this.logger.warn(
        { err, storeId, amount },
        'BudgetService: Redis error, failing open'
      )
    ...
    try {
      ...
      this.db
        ...
        .values({
          ...
          date,
          interventionsSent: 1,
          revenueRecovered: String(amount),
        })
        .onConflictDoUpdate({
        ...
        .catch((err) =>
          this.logger.warn(
            { err, storeId },
            'BudgetService: store_usage increment failed (non-critical)'
          )
        )
    } catch (err) {
      this.logger.warn(
        { err, storeId, amount },
        'BudgetService: reconcile failed (non-critical)'
      )
    }
  }
}
