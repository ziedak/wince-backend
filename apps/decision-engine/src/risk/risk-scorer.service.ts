...
export class RiskScorerService {
  ...
  async score(
    ...
    try {
      ...
      const resolvedType: InterventionType = onnxDriven ? inferenceResult!.type! : ruleResult.type;
      const resolvedChannel: InterventionChannel = onnxDriven ? inferenceResult!.channel! : ruleResult.channel;
      const resolvedValue: number = onnxDriven && inferenceResult!.value !== undefined
        ? inferenceResult!.value
        : ruleResult.value;

      // ── 4. Multi-session weighted aggregate ────────────────────────────
      const userRisk = await this.aggregateUserRisk(userId, event.sid, triggerScore, event.cart_value ?? 0);
      ...
      this.logger.warn({ err, userId, sid: event.sid }, 'RiskScorerService: scoring error — applying fallback ladder');
  ...
  private async aggregateUserRisk(
    ...
    try {
      sessionIds = await raw.zrangebyscore(`user_sessions:${userId}`, cutoff, '+inf');
    } catch {
    ...
    try {
      ...
      hashes = (results ?? []).map(([, v]) => (v as Record<string, string> | null));
    } catch {
      return triggerScore;
    }
    ...
    for (let i = 0; i < sessionIds.length; i++) {
      ...
      const minutesSinceActive = lastActivity > 0
      ...
      const sessionRisk = sid === triggerSid
          ? triggerScore
   ...
   */
  private heuristicRisk(isFrustrated: boolean, rageClicks: number, cartValue: number): number {
    if (isFrustrated && rageClicks > 2) return 0.85;
    if (isFrustrated) return 0.70;
    if (rageClicks > 2) return 0.65;
    if (cartValue > 0) return 0.50;
    return 0.35;
  }
   ...
   */
  private async registerSession(userId: number, sessionId: string, cartValue: number): Promise<void> {
    ...
    try {
      ...
      this.logger.warn({ err, userId, sessionId }, 'RiskScorerService: registerSession failed (non-fatal)');
    }
   ...
   */
  async writeUserRisk(userId: number, storeId: number, score: number): Promise<void> {
    try {
      await this.redis.getRedis()
        .pipeline()
      ...
      this.logger.warn({ err, userId, storeId }, 'RiskScorerService: writeUserRisk failed (non-fatal)');
    }
  }
}
