...
export class RedisClient {
  ...
  async safePublish(channel: string, message: string): Promise<number> {
    if (!channel || typeof channel !== 'string') {
      this.logger.warn(`Invalid channel provided to safePublish channel:${channel}`);
      return 0;
    }
    ...
    if (channel.length > 256) {
      this.logger.warn(`Channel name too long for safePublish channelLength:${channel.length}`);
      return 0;
    }

    if (message.length > 1024 * 1024) {
      // 1MB limit
      this.logger.warn(`Message too large for safePublish messageLength:${message.length}`);
      return 0;
    }
